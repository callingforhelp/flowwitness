...
| Public backend / pilot | Shared ARM64 EC2 pilot is reachable through the named `flowwitness-pilot.useflinter.com` outbound Cloudflare Tunnel. [Pilot 02](PILOT-02.md) records a fresh public-route release-A/release-B replay, retained failed evidence, stale withdrawal, repaired publication, signed delivery deduplication, private PNG access, bilingual queries, image deletion and restart recovery. The original free InsForge Compute endpoint remains the fallback and its 512 MB browser gate is open. No external customer pilot or production certification has been performed. |
