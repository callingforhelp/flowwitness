...
| Container | Docker daemon unavailable on development Mac. CI container build/startup check configured; see linked workflow for live outcome |
...
CI: [Runtime checks](https://github.com/callingforhelp/flowwitness/actions/workflows/runtime.yml). Public concept site deployment is a separate workflow and does not establish backend deployment.

中文：本地测试证明实际服务、浏览器、截图、发布与查询闭环，使用临时数据和合成报表。13 项单元/API/生命周期/概念演示测试通过；真实 Chromium 集成、独立进程 API 验收、中英文操作台与凭据重试也通过。未声称外部客户应用、宿主原生插件或生产部署已经验证。容器结果以 CI 实际记录为准。
