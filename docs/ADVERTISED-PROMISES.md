...
| Offer a public customer-service endpoint or hosted cloud. | The authenticated Node/Playwright service is live for sponsor review at the [hosted backend](HOSTED-BACKEND.md), with InsForge-backed state and private artifact bytes; the [Cloudflare Worker preview](https://flowwitness-preview.dave-z.workers.dev/) remains a read-only concept surface. GitHub Pages remains a simulated concept demo. | The shared ARM64 EC2 route passes public synthetic browser verification, auth, image intake, state restart recovery and API paths. Its accountless tunnel is temporary; the free 512 MB fallback still cannot create a Chromium page reliably. Production browser verification, stable ingress, tenancy, custom-domain operations and external adoption remain open. |
