...
| Offer a public customer-service endpoint or hosted cloud. | The service is local or self-hosted, one application per instance, with admin/support credentials in authenticated mode. GitHub Pages is only a simulated concept demo. | A deployment owner, TLS, private storage, tenant model, operational budget, and an authenticated production validation. |

## The next pilot gate

The internal fixture pilot is complete and documented in
[Pilot 01](PILOT-01.md). The next useful step is one external workflow with no
sensitive data:
