...
对外绑定需两个不同的强随机令牌，建议至少 32 字符。不要公开，操作台只在内存保存。`npm start` 不会自动读 `.env`；显式加载可用 `node --env-file=.env bin/flowwitness.mjs serve`。外部部署需要 TLS、可信来源、私密持久存储和 Chromium 环境。客服令牌只给会验证客户角色的可信聊天后端，不放在公共网页中。当前不支持多租户托管。
