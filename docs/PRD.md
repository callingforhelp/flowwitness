...
| S1-02 | P0 | A complete Git range marks linked and shared workflows as possibly affected, reports unknown coverage, and a signed GitHub push is accepted only once and with a complete change set. | **Partial.** Impact analysis, source relationships, signature validation and deduplication are implemented and tested locally. | Configure the hosted webhook, document delivery retries, and add an operator-controlled digest/scheduler if pilot cadence requires it. |
| S1-03 | P0 | A verification run checks deployment identity before and after bounded browser actions, retains redacted private evidence, and only an explicit operator call can publish a current matching release. | **Blocked for hosted use; implemented locally.** Real Chromium release-A/release-B failure and repair pass locally and in CI. The hosted 512 MB Compute machine cannot reliably create a page. | Move to a browser-capable memory tier or approved remote browser adapter, then repeat the hosted two-release run. |
| S1-04 | P0 | Missing facts create one review question per workflow/revision; operators resolve it before publication. Issues/messages and knowledge entries preserve concise bilingual provenance and never accept hidden chain of thought or credentials. | **Partial.** Legacy questions and the issues/knowledge modules are implemented with local tests and publication gates. Hosted module routes still need explicit support conversation binding. | Move module repository state to a durable hosted adapter and configure trusted support bindings; add scheduled summaries only if a pilot demonstrates the need. |
...
| S1-08 | P0 | A hosted HTTPS service provides separate admin/support auth, durable workflow state and private artifacts, restart recovery, retention, backups and operational health. | **Partial hosted pilot.** InsForge-backed legacy workflow state and artifact bytes survive a machine replacement; health, setup, image and auth checks pass. The module repository is still local to the Compute machine. | Choose durable storage for modules, add backup/restore and alerts, configure a custom domain/TLS policy, and decide retention/tenancy. |
...
| Which durable adapter should store module records: InsForge RPC tables, Postgres, or another approved store? | Engineering | Yes for hosted Stage 1 data |
...
1. Close S1-03 and S1-08: browser capacity, durable module state and basic
   hosted operations.
