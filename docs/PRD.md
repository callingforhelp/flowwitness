...
- The hosted release-A/release-B browser loop passes on the selected compute or
  ...
  browser check through the named `flowwitness-pilot.useflinter.com` Cloudflare
  route; this remains sponsor-pilot evidence rather than production certification.
...
- Named custom-domain/TLS ingress is live for the sponsor pilot; rate limits,
  alerting and a production operator runbook remain required.
...
named sponsor-pilot ingress, legacy state/artifact restart and basic health
portions. It does not satisfy the full production ingress policy,
backup/restore, or the external-pilot portion, so it must be described as a
hosted pilot backend.

## Open questions

| Question | Owner | Blocking? |
| --- | --- | --- |
| Which browser capacity and ingress policy should production use after the shared EC2 pilot? | Stakeholder/operations | Yes for production certification |
...
1. Keep S1-03 closed for the shared EC2 synthetic pilot and close the remaining
   S1-08 operations gates: backup/restore, alerting and policy.
