...
test('resolved impact receipts dedupe only within the same source and workflow revision', async t => {
 ...
 const input = {changed_paths: ['fixtures/reports.html'], source_revision: 'source-1'};
 const first = (await app.service.impact(input)).questions[0];
 assert.equal((await call(`/v1/questions/${first.id}/resolve`, 'POST', {answer: 'Reviewed'})).status, 200);
 ...
 assert.notEqual((await app.service.impact({...input, source_revision: 'source-2'})).questions[0].id, first.id);
...
test('pruning removes expired bytes and publications, keeps fresh artifacts and run history', async t => {
 ...
 const run = {id: 'run', workflow_id: 'export-report', revision: app.store.data.workflows['export-report'].revision,
  status: 'passed', deployment: app.store.data.deployment, expires_at: future, steps: [{artifact_id: old}]};
 app.store.data.runs.run = run;
 await app.service.publish('export-report', 'run');
 const query = {application: app.config.application, question: 'Export report', context: {role: 'admin'}};
...
test('orderly reopen prunes persisted expiry, recovers interrupted jobs and refuses another writer', async t => {
 const root = await fs.mkdtemp(path.join(os.tmpdir(), 'flowwitness-lifecycle-'));
 let app;
 t.after(async () => { if (app) await app.close(); await fs.rm(root, {recursive: true, force: true}); });
 ...
 const file = path.join(app.store.private, app.store.data.artifacts[id].file);
 app.store.data.artifacts[id].expires_at = expired;
 app.store.data.publications.push({id: 'expired-publication', expires_at: expired});
 ...
 const persisted = JSON.parse(await fs.readFile(path.join(app.store.private, 'state.json'), 'utf8'));
