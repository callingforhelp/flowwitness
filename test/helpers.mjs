...
export async function fixture(t, options = {}) {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "flowwitness-"));
  const app = await startServer({ root, port: 0, ...options });
  t.after(async () => {
    await app.close();
    await fs.rm(root, { recursive: true, force: true });
  });
  const call = async (p, method = "GET", body, headers = {}) => {
    const r = await fetch(app.config.origin + p, {
      method,
      headers: {
...
export async function waitJob(call, id) {
  const end = Date.now() + 100000;
  while (Date.now() < end) {
    const { data } = await call("/v1/jobs/" + id);
