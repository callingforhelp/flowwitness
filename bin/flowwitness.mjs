...
const usage='Commands: serve, init, validate <file>, import <file>, list, impact --base <ref> --head <ref>, verify <id>, query <question>';
...
try{
 let result;
 if(['validate','import','verify','query'].includes(command)&&!args.length)throw new Error('Missing argument. '+usage);
 if(['--help','-h','help'].includes(command)){console.log(usage);}
 else if(command==='serve'){const app=await startServer();console.log(`FlowWitness listening on ${app.config.origin} (${app.config.adminToken?'authenticated':'local trusted-user'} mode)`);let stopping=false;for(const signal of ['SIGINT','SIGTERM'])process.on(signal,async()=>{if(stopping)return;stopping=true;await app.close();process.exit(0);});}
 else if(command==='validate'){result={workflow:workflow(JSON.parse(await fs.readFile(args[0],'utf8')),application)};}
 ...
 else if(command==='verify'){result=await call(`/v1/workflows/${encodeURIComponent(args[0])}/verify`,'POST',{});const deadline=Date.now()+15*60*1000;while(['queued','running'].includes(result.job.status)){if(Date.now()>deadline)throw new Error('Verification wait timed out; inspect job '+result.job.id);await new Promise(r=>setTimeout(r,250));result=await call('/v1/jobs/'+result.job.id);}if(result.job.status!=='passed')process.exitCode=1;}
 else if(command==='query'){result=await call('/v1/query','POST',{application,question:args.join(' '),context:{role:process.env.FLOWWITNESS_ROLE||'admin',locale:process.env.FLOWWITNESS_LOCALE||'en'}});}
