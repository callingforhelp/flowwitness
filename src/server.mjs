...
const publicJob=j=>Object.fromEntries(Object.entries(j).filter(([k])=>!['workflow','deployment','key'].includes(k)));
export async function createServer(options={}){
 ...
 if(!authenticated&&mutation&&p!=='/v1/webhooks/github')demand(['dashboard','cli'].includes(req.headers['x-flowwitness-client']),'Client header required','forbidden',403);
 const now=Date.now(),key=req.socket.remoteAddress;let rate=rates.get(key);if(!rate||now-rate.at>60000){rate={at:now,count:0};rates.set(key,rate);}demand(++rate.count<=600,'Rate limit exceeded','rate_limited',429);
 let raw=Buffer.alloc(0),b={};if(mutation){const chunks=[];let size=0;for await(const chunk of req){size+=chunk.length;demand(size<=3000000,'Request body too large','body_too_large',413);chunks.push(chunk);}raw=Buffer.concat(chunks);if(raw.length){try{b=JSON.parse(raw);}catch{throw new Fault('invalid_json','Invalid JSON');}}demand(b&&typeof b==='object'&&!Array.isArray(b),'JSON object required');}
 const get=(collection,id)=>{const item=store.data[collection][id];demand(item,'Not found','not_found',404);return item;};let m;
 if(p==='/health'&&req.method==='GET')return json(200,{status:'ok',mode:authenticated?'authenticated':'local',application:config.application});
 if(p==='/v1/workflows'&&req.method==='GET')return json(200,{workflows:Object.values(store.data.workflows).map(w=>service.enriched(w))});
 if(p==='/v1/workflows'&&req.method==='POST')return json(201,{workflow:await service.put(b)});
 if((m=p.match(/^\/v1\/workflows\/([a-z0-9-]+)$/))){const w=get('workflows',m[1]);if(req.method==='GET')return json(200,{workflow:service.enriched(w)});if(req.method==='DELETE'){delete store.data.workflows[w.id];store.data.publications=store.data.publications.filter(x=>x.workflow.id!==w.id);await fs.rm(path.join(store.root,'workflows',w.id+'.json'),{force:true});await store.save();return json(200,{deleted:true});}}
 if(p==='/v1/deployment'){if(req.method==='GET')return json(200,{deployment:store.data.deployment});if(req.method==='PUT')return json(200,{deployment:await service.setDeployment(b)});}
 if((m=p.match(/^\/v1\/workflows\/([a-z0-9-]+)\/verify$/))&&req.method==='POST')return json(202,{job:publicJob(await service.enqueue(m[1]))});
 if((m=p.match(/^\/v1\/workflows\/([a-z0-9-]+)\/publish$/))&&req.method==='POST')return json(200,{publication:await service.publish(m[1],b.run_id)});
 if(p==='/v1/jobs'&&req.method==='GET')return json(200,{jobs:Object.values(store.data.jobs).map(publicJob)});
 if((m=p.match(/^\/v1\/jobs\/([a-z0-9-]+)$/))&&req.method==='GET')return json(200,{job:publicJob(get('jobs',m[1]))});
 if((m=p.match(/^\/v1\/runs\/([a-z0-9-]+)$/))&&req.method==='GET')return json(200,{run:get('runs',m[1])});
 if(p==='/v1/questions'&&req.method==='GET')return json(200,{questions:Object.values(store.data.questions)});
 if((m=p.match(/^\/v1\/questions\/([a-z0-9-]+)\/resolve$/))&&req.method==='POST'){demand(typeof b.answer==='string'&&b.answer.trim()&&b.answer.length<=2000,'Answer required');const q=get('questions',m[1]);q.answer=b.answer;q.status='resolved';await store.save();return json(200,{question:q});}
 if(p==='/v1/impact'&&req.method==='POST')return json(200,await service.impact(b));
 ...
 if(p==='/v1/graph'&&req.method==='GET'){const nodes=[],edges=[];for(const w of Object.values(store.data.workflows)){nodes.push({id:w.id,type:'workflow',label:w.title});for(const s of [...w.source_paths,...w.shared_paths]){nodes.push({id:s,type:'source',label:s});edges.push({from:s,to:w.id,type:'declared_source'});}const screen=w.id+':'+w.start_path;nodes.push({id:screen,type:'screen',label:w.start_path});edges.push({from:w.id,to:screen,type:'starts_at'});for(const s of w.steps){nodes.push({id:w.id+':'+s.id,type:'step',label:s.instruction});edges.push({from:w.id,to:w.id+':'+s.id,type:'contains'});}}for(const r of Object.values(store.data.runs)){nodes.push({id:r.id,type:'run',label:r.status});edges.push({from:r.workflow_id,to:r.id,type:'replay'});}return json(200,{nodes:[...new Map(nodes.map(n=>[n.id,n])).values()],edges});}
 if(req.method==='GET'&&p==='/fixture/version')return json(200,{version:store.data.fixture,source_revision:'fixture-'+store.data.fixture});
 if(req.method==='GET'&&(p==='/fixture/'||p==='/fixture')){const html=(await fs.readFile(path.join(moduleRoot,'fixtures/reports.html'),'utf8')).replace('{{MORE_HIDDEN}}',store.data.fixture==='v1'?'hidden':'').replace('{{MENU_HIDDEN}}',store.data.fixture==='v2'?'hidden':'');res.writeHead(200,{'content-type':'text/html'});return res.end(html);}
 if(req.method==='GET'&&!p.startsWith('/v1/')){const base=p.startsWith('/app')?'app':'site';let rel=decodeURIComponent(base==='app'?p.slice(4):p);if(rel==='')rel='/';if(rel.endsWith('/'))rel+='index.html';const file=path.resolve(moduleRoot,base,'.'+rel);demand(file.startsWith(path.join(moduleRoot,base)+path.sep),'Invalid path');const ext=path.extname(file);demand(['.html','.css','.js','.mjs','.svg','.png','.ico','.webp'].includes(ext),'Not found','not_found',404);const data=await fs.readFile(file).catch(()=>{throw new Fault('not_found','Not found',404);});res.writeHead(200,{'content-type':({'.html':'text/html','.css':'text/css','.js':'text/javascript','.mjs':'text/javascript','.svg':'image/svg+xml','.png':'image/png','.webp':'image/webp'})[ext]||'application/octet-stream','x-content-type-options':'nosniff'});return res.end(data);}
 throw new Fault('not_found','Not found',404);
 }catch(e){json(e.status||500,{error:{code:e.code||'internal_error',message:e.status?e.message:'Request failed'},request_id});}});
